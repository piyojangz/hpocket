                        ...
                        <div class="col-md-12">
                            <div class="panel">
                                ...
                                <div class="table-responsive">
                                    <?php if(isset($err)):?>
                                    <?php foreach ($err as $index => $item): ?>  
                                        <table class="table table-hover manage-u-table">
                                            <thead>
                                                <tr>
                                                    <th style="width:50px">#</th>
                                                    <th  class="text-left">Endpoint (<?= $index ?>)</th>  
                                                    <th class="text-left">Create time</th> 
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <?php foreach (json_decode($this->firebase->get("LogProd/Error/$index")) as $index2 => $item2): ?> 
                                                    <tr>
                                                        <td><a target="_blank" href="<?= base_url("logprod/errdetail/$index/$index2")?>" class="btn   btn-default">view</a></td>
                                                        <td style="width:100px" class="text-left"><?= $item2->errordetail->endpoint ?></td>     
                                                        <td   class="text-left"><?= $item2->errordetail->createtime ?></td>  
                                                    </tr>
                                                <?php endforeach; ?>
                                            </tbody>
                                        </table>
